...
module.exports = {
  execute: async (instance, message, args) => {
    ...
    if (instance.shared["knowsClans"] !== allClanIds.length) {
      ...
      let query = `SELECT id, guild_id FROM SERVERS WHERE ${allClanIds
        .map(g => `guild_id='${g}'`)
        .join(" OR ")}`;
      const { rows } = await instance.database.pool.query(query);
      rows.forEach(srv => (instance.serverIds[srv.guild_id] = srv.id));
      instance.shared["knowsClans"] = allClanIds.length;

      const missing = allClanIds.filter(
    ...
    for (const [i, clan] of clans.entries()) {
      ...
      } ${clan.name} - ${clan.camt}/${clan.samt} claims (${clan.percent}%)`;
